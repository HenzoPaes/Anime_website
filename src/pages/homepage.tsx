...
// Cores do banner de anúncio
const ANNOUNCEMENT_STYLES = {
  info:    { bg: "rgba(59,130,246,0.12)", border: "rgba(59,130,246,0.3)", icon: "ℹ️", color: "#60a5fa" },
  warning: { bg: "rgba(245,158,11,0.12)", border: "rgba(245,158,11,0.3)", icon: "⚠️", color: "#fbbf24" },
  success: { bg: "rgba(34,197,94,0.12)",  border: "rgba(34,197,94,0.3)",  icon: "✅", color: "#4ade80" },
};
...
export default function HomePage() {
  ...
  // PageConfig — carregado do GitHub
  ...
  // Título da Página — vem do pageconfig
  useEffect(() => {
    document.title = config.siteTitle ?? "AnimeVerse — Seu portal de animes";
  }, [config.siteTitle]);
  ...
  // Anime em destaque: prioriza featuredAnimeId do config, senão maior nota
  const featured = useMemo(() => {
    if (!animes.length) return null;
    if (config.featuredAnimeId) {
      const pinned = animes.find(a => a.id === config.featuredAnimeId);
      if (pinned) return pinned;
    }
    return [...animes].filter(a => a.rating).sort((a, b) => (b.rating ?? 0) - (a.rating ?? 0))[0];
  }, [animes, config.featuredAnimeId]);

  // Gêneros únicos
  const allGenres = useMemo(() => {
    const s = new Set<string>();
    animes.forEach(a => (a.genres || []).forEach(g => s.add(g)));
    // Pinned genres do config aparecem primeiro
    const pinned = (config.pinnedGenres ?? []).filter(g => s.has(g));
    const rest    = Array.from(s).filter(g => !pinned.includes(g)).sort();
    return [...pinned, ...rest];
  }, [animes, config.pinnedGenres]);
  ...
  // Badge e CTA do hero vêm do config
  const heroBadge = config.heroBadgeText ?? "⭐ Melhor avaliado";
  const heroCta   = config.heroCtaText   ?? "Assistir agora";
  const catalogTitle = config.catalogTitle ?? "Todos os Animes";
  const showRandom   = config.showRandomButton !== false;

  // Anúncio
  const ann = config.announcement;
  const showAnn = !!(ann?.enabled && ann?.text && !announcementDismissed);
  const annStyle = ANNOUNCEMENT_STYLES[ann?.type ?? "info"];

  return (
    <motion.div variants={PAGE} initial="initial" animate="animate" exit="exit" className="pb-12">

      {/* ========================================== */}
      {/* BANNER DE ANÚNCIO (pageconfig.announcement) */}
      {/* ========================================== */}
      <AnimatePresence>
        {showAnn && (
          <motion.div
            ...
            style={{
              background: annStyle.bg,
              borderBottom: `1px solid ${annStyle.border}`,
              overflow: "hidden",
            }}
          >
            <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
              <span style={{ fontSize: 16, flexShrink: 0 }}>{annStyle.icon}</span>
              <p style={{ fontSize: 13, color: annStyle.color, flex: 1, fontWeight: 500 }}>
                {ann!.text}
              </p>
              {ann?.dismissible !== false && (
                <button
                  ...
                  style={{
                    ...
                    color: annStyle.color, opacity: 0.6,
        ...
        <section className="relative overflow-hidden min-h-[420px] flex items-end">
          <div className="absolute inset-0">
            <img
              ...
              alt=""
              className={`w-full h-full object-cover scale-110 opacity-70 ${config.featuredBannerBlur ? "blur-sm" : ""}`}
            />
          ...
          <div className="relative max-w-7xl mx-auto px-4 py-16 w-full">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex items-center gap-2 mb-3">
              ...
              {/* Badge 18+ no hero se adulto */}
              {(featured as any).adultContent && (
                <span style={{
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="flex flex-wrap items-center gap-3 mb-4 text-sm text-gray-400 font-medium">
              ...
              {featured.genres?.slice(0, 3).map(g => (
                <span key={g} className="capitalize bg-white/10 px-2 py-0.5 rounded-full text-xs">{g}</span>
              ))}
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="flex gap-3 flex-wrap">
              ...
              {showRandom && (
                <button
                  ...
                  className={`btn-ghost flex items-center gap-2 px-6 py-3 ${randFlash ? "scale-95 opacity-60" : ""} transition-all duration-300 hover:bg-white/10`}
                >
                  🎲 Anime aleatório
                </button>
              )}
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <div className="flex gap-2 flex-shrink-0">
                  ...
                  >
                    ...
                    Filtros
                    {activeFilters > 0 && (
                      <span className="w-5 h-5 bg-brand-500 rounded-full text-white text-xs font-bold flex items-center justify-center">{activeFilters}</span>
                    )}
                  </button>
                </div>
              </div>
                    ...
                    <div className="glass rounded-2xl p-6 space-y-5">
                      ...
                      <div className="flex flex-wrap gap-2 items-center">
                        ...
                        {AUDIO_OPTIONS.map(({ value, label }) => (
                          <button key={value} onClick={() => setAudioF(value)}
                            className={`btn-ghost text-xs py-1.5 px-3 transition-colors ${audioF === value ? "border-brand-500 text-brand-400 bg-brand-500/10" : "hover:border-white/20"}`}>{label}</button>
                        ))}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2 items-center">
                        ...
                        {STATUS_OPTIONS.map(({ value, label }) => (
                          <button key={value} onClick={() => setStatusF(value)}
                            className={`btn-ghost text-xs py-1.5 px-3 transition-colors ${statusF === value ? "border-brand-500 text-brand-400 bg-brand-500/10" : "hover:border-white/20"}`}>{label}</button>
                        ))}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2 items-center">
                        ...
                        {RATING_OPTIONS.map(v => (
                          <button key={v} onClick={() => setMinRating(v)}
                            className={`btn-ghost text-xs py-1.5 px-3 transition-colors ${minRating === v ? "border-brand-500 text-brand-400 bg-brand-500/10" : "hover:border-white/20"}`}>
                            {v === 0 ? "Todos" : `≥ ${v}★`}
                          </button>
                        ))}
                      </div>

                      {/* Gêneros — pinned aparecem em roxo */}
                      {allGenres.length > 0 && (
                        <div className="flex flex-wrap gap-2 items-start">
                          <span className="text-xs font-bold text-gray-500 uppercase w-16 flex-shrink-0 mt-2">Gênero</span>
                          <div className="flex flex-wrap gap-2">
                            {allGenres.map(g => {
                              const isPinned = (config.pinnedGenres ?? []).includes(g);
                              const isSelected = selGenres.includes(g);
                              return (
                                <button
                                  key={g}
                                  onClick={() => setSelGenres(p => p.includes(g) ? p.filter(x => x !== g) : [...p, g])}
                                  className={`text-xs py-1 px-3 rounded-full border transition-all capitalize ${
                                    isSelected
                                      ? "bg-brand-500/20 border-brand-500 text-brand-300"
                                      : isPinned
                                        ? "border-purple-500/40 text-purple-400 hover:border-purple-400"
                                        : "border-white/10 text-gray-400 hover:border-white/30 hover:text-white"
                                  }`}
                                >
                                  {isPinned && !isSelected && <span style={{ marginRight: 4 }}>📌</span>}
                                  {g}
                                </button>
                              );
                            })}
